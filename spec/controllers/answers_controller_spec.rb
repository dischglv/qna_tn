...
RSpec.describe AnswersController, type: :controller do
        ...
        context 'votes for the first time' do
          it 'changes number of answer votes' do
            expect do
              ...
              answer.reload
            end.to change(answer.votes.where(value: true), :count).by(1)
          end

          it 'renders votes in json' do
            patch :vote_for, params: { question_id: question, id: answer }, format: :json

            expect(response.body).to eq([answer.votes.first].to_json)
          end
        ...
        context 're-votes' do
          ...
          it 'does not changes number of votes' do
            expect do
              patch :vote_for, params: { question_id: question, id: answer }, format: :json
              answer.reload
            end.to_not change(answer.votes.where(value: true), :count)
          end
        ...
        context 'votes for the first time' do
          it 'changes number of answer votes' do
            expect do
              ...
              answer.reload
            end.to change(answer.votes.where(value: false), :count).by(1)
          end
        ...
        context 're-votes' do
          ...
          it 'does not changes number of votes' do
            expect do
              patch :vote_against, params: { question_id: question, id: answer }, format: :json
              answer.reload
            end.to_not change(answer.votes.where(value: false), :count)
          end
  ...
  describe 'DELETE #cancel_vote' do
    ...
    context 'User voted before' do
      let!(:vote) { create(:vote, votable: answer, user: user2, value: true) }
